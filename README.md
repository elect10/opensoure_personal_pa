...
# 구현 기능 ( !!!! 모든 기능은 모바일 화면비 (393* 853) 에 맞춰 구현 되었습니다.)
+ 로그인
+ 회원가입
+ 포트폴리오 작성 및 수정 페이지
...
```
5. http://localhost:3000 으로 진입후 , 회원가입 , 로그인을 진행한다. (id 는 본인의 학번을 기입, 접속시 웹 브라우저의 화면비 구성을 393 * 853 으로 조정)
```
 http://localhost:3000
```
### 설치후 접속 (mac ver14 기준)
...
# 코드 설명 
(코드의 양이 너무 방대해 디자인적 코드는 배제하고, 핵심 기능만을 설명합니다.)
...
## App.js 
+ 각각 코드들에 대한 라우팅을 정리해둔 코드. 
+ react-router-dom 을 이용했다.
...
# For next Developer
+ react 코드는 my-app 안에 있습니다. 
+ components 폴더 안의 코드는 보통 routes 폴더 안에 있는 페이지 구성 코드가 사용하는 컴포넌트 함수입니다.
+ asset/image 에는 리액트에서 사용하는 이미지가 있습니다.
